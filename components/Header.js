import { MicrophoneIcon, SearchIcon, XIcon } from '@heroicons/react/solid'
import Image from 'next/image'
import { useRouter } from 'next/router'
import React, { useRef } from 'react'
import Avatar from './Avatar'
import HeaderOptions from './HeaderOptions'

function Header() {
    const router = useRouter()

    // to point to input box
    const searchInputRef = useRef(null)

    const search = (e) => {
        e.preventDefault()

        const term = searchInputRef.current.value

        if (!term) return

        router.push(`/search?term=${term}`)
    }

    return (
        <header className='top-0 sticky z-50 bg-white'>
            <div className='flex w-full p-6 items-center'>
                <Image src={'https://www.google.co.uk/images/branding/googlelogo/2x/googlelogo_color_272x92dp.png'} objectFit='contain' height={40} width={120} className='cursor-pointer' onClick={() => router.push('/')} />

                <form className='flex flex-grow border border-gray-200 rounded-full shadow-lg max-w-3xl ml-10 mr-5 px-6 py-3'>
                    {/* ref={searchInputRef} >>> getting the input search */}
                    <input placeholder={router.query.term} type={'text'} ref={searchInputRef} className='flex-grow w-full focus:outline-none' />

                    <XIcon className='h-7 sm:mr-3 text-gray-500 cursor-pointer hover:scale-125 active:scale-90 hover:bg-gray-100 
                rounded-full transform transition duration-100 ' onClick={() => { searchInputRef.current.value = "" }} />
                    {/* searchInputRef.current.value = "" >> it will clear onClick */}

                    <MicrophoneIcon className='h-6 mr-3 hidden sm:inline-flex text-blue-500 border-l-2 pl-4 border-gray-300' />

                    <SearchIcon className='h-6 text-blue-500 hidden sm:inline-flex' />
                    <button type='submit' hidden onClick={search}>Search</button>
                </form>

                <Avatar className='ml-auto' url={'https://avatars.githubusercontent.com/u/39983195?v=4'} />
            </div>

            {/* Header Options */}
            <HeaderOptions />
        </header>
    )
}

export default Header
